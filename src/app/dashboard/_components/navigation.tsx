...
export default function Navigation() {
  ...
  const isLinkActive = (href: string) => pathname.startsWith(href) && (pathname === href || pathname.startsWith(`${href}/`));
                ...
                {section.items.map((link, linkIndex) => {
                  ...
                  return (
                    <SidebarMenuItem key={linkIndex}>
                      <Link href={link.href!}>
                        <SidebarMenuButton isActive={isDashboardActive()} tooltip={link.tooltip} className="rounded-full">
                            <div className="flex flex-grow items-center gap-3">
                              {link.icon}
